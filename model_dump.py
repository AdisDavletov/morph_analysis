...
class Model(object):
  ...
  def _build_model(self, config):
    ...
    with tf.variable_scope("clfs_part", reuse=tf.AUTO_REUSE):
      self._add_classifiers(config.classifiers, config, inputs if config.embeddings_to_classifiers else None)
    ...
    self.accuracy, self.accuracy_op = self._add_accuracy()
  ...
  def _add_accuracy(self):
    accuracy_op = {}
    accuracy = {}
    for clf in self._predictions:
      if clf != 'word':
        accuracy[clf], accuracy_op[clf] = tf.metrics.accuracy(self._predictions[clf], self.input.input_data[clf])
  ...
  def _add_embed_layer(self, vocab_size, embedding_size, keep_prob):
    with tf.device("/cpu:0"):
      ...
      inputs = tf.nn.embedding_lookup(embedding, self._input.input_data['word']) # BS X SEQ_LEN X EMB_SIZE
  ...
  def _add_rnn_layer(self, inputs, num_layers, keep_prob, hidden_size, cell_type):
    ...
    inputs_ = tf.unstack(inputs, num=self.num_steps, axis=1)
    outputs, self._final_state = tf.nn.static_rnn(cell, inputs_,
  ...
  def _add_classifiers(self, classifiers, config, embeddings=None):
    ...
    if embeddings is not None:
      inputs = tf.concat([prev_output, self._rnn_output, embeddings], axis=2)
    else:
    ...
    inputs = tf.reshape(inputs, shape=[-1, self._rnn_out_size * 2 + (config.embedding_size if embeddings is not None else 0)])
    for name, out_size in classifiers.items():
      logits = self._add_clf_layer(
        inputs=inputs,
        in_size=self._rnn_out_size * 2 + (0 if embeddings is None else config.embedding_size),
        out_size=out_size,
        layer_name=name)
  ...
  def _add_predictions(self):
    ...
    for clf in self._logits: # bs x seq_len x logits_size
      if clf == 'word_target':
        self._predictions['word'] = self.input.input_data['word']
        continue
...
class SmallConfig(object):
  ...
  def __init__(
    ...
    self.embeddings_to_classifiers = embeddings_to_classifiers
...
def run_epoch(session, model, cost_name, is_training=True, eval_op=None, verbose=False, accuracy_logs='accuracy_logs.txt'):
  ...
  total_accuracy = {}
  for clf in model._predictions:
    if clf != 'word':
      total_accuracy[clf] = 0.0
  ...
  for step in range(model.input.epoch_size):
    ...
    for clf in vals['accuracy']:
      total_accuracy[clf] += (vals['accuracy'][clf] * model.batch_size * model.num_steps)
  ...
  with open(str(is_training) + '_' + FLAGS.log_file, 'a') as f:
    ...
    print('end of acuracies', file=f, flush=True)
  return np.exp(costs / iters)

def predict(session, model, classifiers, word_to_id, num_batchs, out_file='predicted.out'):
  tag_mapping = {
    'numform': 'NumForm',
    'case': 'Case',
    'gender': 'Gender',
  ...
  with open(out_file, 'w') as f:
    for i in range(predictions['word'].shape[1]):
      line = {}
      for clf in predictions:
        x = predictions[clf][0, i]
        line[clf] = id_to_word[clf][x]
      if line['word'] == '<eos>':
        print('', file=f)
        idx = 1
        continue
      tags_to_write = []
      for clf in line:
        if line[clf] != f"{clf}_unk" and clf not in ['word', 'pos']:
          tags_to_write.append(f"{tag_mapping[clf]}={line[clf].capitalize()}")
      print(idx, line['word'], line['pos'].upper(), '|'.join(tags_to_write) if tags_to_write else '_', sep='\t', file=f)
      idx += 1
...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    with tf.name_scope("Train"):
      ...
      for clf in m.accuracy_op:
        tf.summary.scalar(f"Training {clf.upper()} Accuracy", m.accuracy_op[clf])
    ...
    with tf.name_scope("Valid"):
      ...
      for clf in mvalid.accuracy_op:
        tf.summary.scalar(f"Validation {clf.upper()} Accuracy", mvalid.accuracy_op[clf])
    ...
    with tf.train.MonitoredTrainingSession(config=config_proto, checkpoint_dir=FLAGS.save_path) as session:
      if not FLAGS.prediction:
        ...
        for i in range(config.max_max_epoch):
          ...
          valid_perplexity = run_epoch(session, mvalid, config.loss_to_view, is_training=False)
